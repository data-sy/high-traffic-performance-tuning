...
> **"페이징 목록을 기본으로"는 이 좌표의 판단이지 보편 정답이 아니다.** 조회 화면이 대개 페이징을
> 쓰고 1:N이 얕은(컬렉션 하나) 이 스키마에서, v3가 *예측 가능성*으로 우세하다는 뜻이다 — batch fetch가
> 어디서나 정석이라는 게 아니다. 같은 기준으로 비페이징 단건은 v4가, 전송·적재가 임계인 핫패스는 v5가
> 이긴다. 워크로드 좌표(페이징 여부·1:N 깊이·적재 필요)가 바뀌면 배선도 바뀐다.
