...
> *"분산 락을 끝까지 구현해 보고도, 이 워크로드의 정답은 DB 행 락(v3)이라고 단정했다.
> 이 리포트는 그 판단과, 그 판단을 스스로 검증·기각한 방법론의 기록이다."*
...
| **성격** | 성과 마이그레이션 보고가 아니라 **학습·방법론 회고**. 주인공은 수치가 아니라 판단과 측정 규율이다. |

> ⚠️ **이 리포트가 끝까지 지킨 한 가지**: 버전 간 throughput을 직접 비교하지 않는다. "어느 락이
> 제일 빠른가"라는 헤드라인을 만들지 않는다. throughput은 각 버전 **정책의 자체 특성**으로만 읽고,
> 비교 합법 축(정합 actual)과 정책 관찰치(503 거절률)만 시각화한다. 이유는 §9에서 못 박는다.
...
회복했고, 사다리 꼭대기(커스텀 분산 락)까지 올라가 반증을 시도한 끝에, 이 워크로드(임계 구간이 곧
...
**왜 사다리인가 — 정합 범위를 한 칸씩 넓히며 각 경합 처리의 성격을 드러내려고.** "제일 고급 기법을
골라 배포했다"가 아니다. 각 칸은 *더 낫고 못한* 단계가 아니라 **서로 다른 조건에서 각자 정답**인 별개
도구다. 사다리를 끝까지 밟은 건 우열을 매기려는 게 아니라, **분산 락이 v3 대비 무엇을 더 주는지(혹은
못 주는지)를 직접 구현해 확인하지 않고는 v3를 정답이라 부를 수 없어서**였다.
...
핀 3종 중 2종은 결정적 실측, Tomcat 1종은 무해 미검증 — 이 **정직한 비대칭**을 숨기지 않고
방법론으로 기록했다. "모든 핀을 런타임 실측했다"가 아니라, **검증으로 구별할 두 시나리오가 있는
핀만** 검증했다(없으면 검증 동기 부재). 측정의 안전망을 먼저 깐다는 게 이런 뜻이다.

**성공 시그니처도 코드가 아니라 데이터로 못 박았다.** 정합 판정의 단일 진실 소스는
...
> 직접 비교가 정당하다. v4의 62는 미달이 아니라 fail-fast의 한도 미충전(§9 클래스 B) — 90s로 늘리면
> 100으로 수렴한다.
...
3. **방법론 자산** — (a) 통제변수 규율로 비교 유효성을 보존했고, (b) 거짓 헤드라인(throughput 직접
   비교)을 회피했으며, (c) 측정 함정을 클래스 A/B로 분류해 자동 판정이 못 거르는 human 오독을 닫았다.
   이 프로젝트는 회고가 방법론 비중이 커도 좋은 종류다 — 그게 차별점이라서.
...
## 9. 메타 회고 — 이 프로젝트의 진짜 차별점

**측정을 함부로 믿지 않는 규율이 결론보다 중요했다.** 세 가지로 요약한다.
...
**(3) 클래스 A vs 클래스 B 함정 구분이 핵심이었다.**

| | 클래스 A — 환경 의존 트랩 | 클래스 B — 정책 의존 해석의 오독 |
|---|---|---|
...
클래스 A는 도구(assert)로 막을 수 있다. **클래스 B는 도구로 못 막는다** — 하네스의 이진 PASS/FAIL은
v4의 `actual=62`를 기계적으로 FAIL로 찍지만, 그건 정합 실패가 아니라 대기 큐 부재의 정상 귀결이고,
duration을 늘리면 100으로 수렴한다. 이 구분을 못 하면 측정값을 정확히 읽고도 결론을 틀린다.
**"이 사람은 측정을 함부로 믿지 않고, 자기 결론을 스스로 검증·기각할 줄 안다"** — 이 리포트가
남기려는 건 그것이다.
