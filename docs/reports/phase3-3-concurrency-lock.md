...
DB 쓰기)에서는 **Redis 분산 락을 기각하고 v3(DB 비관적 행 락)을 정규 발급 경로에 배선**했다. 락↔데이터가
같은 트랜잭션 안에 있어 fencing 공백이 구조적으로 없기 때문이다. 분산 락은 보호 대상이 DB 밖에 있을
때 쓴다.
...
이 한 칸이 "정합 판정은 항상 행 수로"라는 규율의 근거가 됐다. **v1의 FAIL은 의도된 결과다 — 무락
기준선은 초과를 재현해야 하므로, 여기서 PASS가 나면 그게 버그다.**
...
**여기서 v3의 음성 결과(ⓐ/ⓑ)를 짚어둔다.** ⓐ(풀 고갈)/ⓑ(행락 타임아웃) 503 분류는
...
ⓐ/ⓑ만 이 워크로드에서 음성으로, 공백이 아니라 측정된 거동으로 기록한다.
...
맞는 자리는 보호 대상이 DB 밖일 때**다. 분산 락(v4/v5)까지 구현해 비교한 뒤, 이 좌표에선 DB 행
락이 맞다고 판단했다.
