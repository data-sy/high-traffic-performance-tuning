...
### 4.2 유실 축 — 유실은 큐 매체가 아니라 *종료 방식*에 특정된다
...
**유실의 출처는 큐가 아니라 종료 방식이다.** 가장 흔한 오해는 "비동기 큐를 쓰면 배포할
...
드레인) 또는 ack+회수(v6c)로 닫힌다. **그래서 큐 매체보다 종료를 어떻게 다루는가가 유실을 가른다.**
...
비교 금지(접수 지연 vs 처리 지연, 부하 프로필 상이 — 참조만). throughput 헤드라인 막대는 한 장도
그리지 않았다. 합법 비교 축은 정확성(초과발급 0) 하나뿐이다.
