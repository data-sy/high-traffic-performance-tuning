...
> ⚠️ **throughput·지연·503율 버전 간 직접 비교 금지** — 경합 전략이 버전마다 달라(park/DB큐/fail-fast) 같은
> 컬럼이라도 의미가 다르다. 아래 수치는 **outcome별 분리 + 버전 내 정책 관찰치**로만 읽는다("거절"=503-only).
...
> ¹ **v4 정확성 = 각주 분리(오독 방지)**: 공유부하 10s actual=**62**(미충전 — 대기 큐 부재의 fail-fast 정상 귀결,
>   **≠정합 실패**), duration↑ 90s 충전 시 actual=**100**으로 한도 수렴. 합격식 = `≤total`(초과 0) 충족 → **PASS**.
>   표 본체는 "≤total ✓"만 — 62를 "미달=FAIL"로 읽으면 틀림(v4 보고서 닫은 해석).
> ² **v1 FAIL = 의도된 결과**(무락 기준선의 초과 재현). PASS였으면 그게 버그.
> ³ **v3 503=0 = 음성 결과(빈칸 아님)**: ⓐ/ⓑ 분류는 배선·풀포화(active 10/10·pending 189)까지 검증됐으나 이
>   핀·부하서 타임아웃 미발생 → 라우팅 미주행. **DB 행락이 connTO 30s 안에서 경합을 거절(503) 아닌 지연(p95 2.6s)
>   으로 흡수**한 증거(경합 흡수 축의 핵심). 503 라우팅·errorCode 분류 코드 자체는 v4/v5 ⓒ로 실주행 증명됨.

### 서사 축 (3겹 — "어느 락이 빠른가"가 아니다)
1. **정합 범위**: 없음(v1·초과) → 단일 JVM 모니터(v2) → 공유 DB행(v3) → 외부 Redis(v4·v5). 분산 정합이 사다리 값.
   단 **단일 JVM에선 v2~v5 다 정합** — v4/v5의 다중 인스턴스 정합은 본 측정서 미실증(↓ 한계).
2. **경합 흡수**("직렬화 비용이 어디서 나오나"): 무시(v1) → **park 무한대기**(v2) → **DB 대기열**(v3, 503=0 = 경합을
   지연으로 흡수, ⓐ/ⓑ 음성이 그 증거) → **fail-fast 거절**(v4·v5, 503율 ~100% ⓒ). 블로킹은 경합을 *지연*으로,
   fail-fast는 *거절*로 배출 — 정책이 다른 두 숫자라 단일 우승자 없음.
3. **갱신 정책**(v4 워치독 vs v5 고정TTL): 워치독이 사주는 것 = 만료-중간탈취 차단. **단 정상 부하 throughput으론
   격리 불가**(발견8 — hold 수 ms ≪ TTL이라 고정TTL이 중간 만료 안 됨, v5>v4 차이는 대기/락구현 아티팩트). 갱신
   축의 진짜 효과는 **스톨 데모(hold>TTL)로만** 드러남 — 그리고 거기서도 **둘 다 fencing-safe 아님**(actual=2).

### ★ throughput 버전 간 직접 비교 금지 — 그 이유 (3-3-2 통제 재실험 결론 흡수)
- 버전 간 throughput 격차(예: v3 317 > v2 264, v5 28 > v4 6)는 **락 위치 때문이 아니라 경합 전략이 같이 움직인
  교란**이다(v1/v2/v3=대기 흡수, v4/v5=fail-fast 거절). 경합을 **유한대기로 통일하면** 격차가 **작은 단조 락-위치
  비용으로 수렴**(락없음 > JVM ≳ DB > 외부 Redis, near-parity)하고 전부 정합·거절 0이 된다.
- 즉 raw 격차의 **지배 변수는 락 기질이 아니라 경합 처리 방식** → throughput을 "어느 락이 빠른가"로 읽지 않는다.
  v3>v2·v5>v4는 전부 **관측·미증명**(단일 런, 교란 미통제).

### §6 프로덕션 배선 = v3 (DB 비관적 행 락) — 결정과 근거
- **선택**: 정규 발급 경로 `POST /api/coupons/issue`(무버전) → **V3** 명시 위임. v1/v2/v4/v5는 비교용 공존(운영 무관).
- **핵심 근거 — 락↔데이터 동치(원자성)**: 이 워크로드의 임계영역 = **DB 쓰기**(coupon 행 UPDATE + coupon_issue
  INSERT). 보호 대상(coupon 행)이 **이미 DB 안**에 있다. v3는 락(FOR UPDATE 행 락)과 데이터가 **같은 DB·같은
  트랜잭션** 안에 있어 **원자적** — 락은 커밋 시 풀리므로 만료-중간탈취 자체가 불가, **fencing 공백이 구조적으로
  없다.** 락을 Redis로 빼면 **락↔데이터가 다른 시스템에 분리**되며 그 공백이 열린다(**스톨 데모 actual=2가 바로
  그 귀결**).
- 따라서 v3는 v4/v5보다 단지 "충분"한 게 아니라 **이 워크로드에선 더 안전하다.** Redis 분산 락은 보호 대상이
  **DB 밖**일 때(외부 API 호출 멱등성, 비-DB 자원 보호) 의미가 있지, **DB 행 보호엔 락을 데이터에서 떼어내는
  역효과**(fencing 공백 + 단일 Redis SPOF)만 더한다.
- **대가**: v3의 풀 압박(active 10/10·pending 189)은 측정·튜닝 가능한 비용(풀·타임아웃 핀)이며 본 워크로드 규모서
  수용 가능. → "**왜 분산 락(트렌드)을 안 썼나**"는 트렌드 무지가 아니라 **워크로드 적합성 판단**. 맥락이 바뀌면
  (Redis가 이미 핵심 캐시로 깔림 + DB 부하 분리 절실 + 비-DB 임계영역) v4로 이전.
- **⚠️ 오독 방지**: Redis ≠ 스케일아웃 전제. v3는 공유 DB 행락이라 **이미 다중 인스턴스 정합**(인스턴스를 늘려도
  모두 같은 행락 경유 → 경계 초월). v4/v5가 v3 넘어 더해주는 건 정합의 *범위*가 아니라 *비-DB 임계영역 보호*뿐.

### 한계 (범위 한정 + 알려진 한계)
- **범위 한정**: 모든 결론은 **단일 핫키(couponId=1) + 단일 JVM** 가정에 묶인다. "v4/v5 거절률 ~100%"는 단일
  핫키+즉시실패의 귀결이지 분산 락 일반의 성질이 아니다.
- **다중 인스턴스 미검증 = 알려진 한계 · 다음 후보 · 이번 스코프 아님**: v4/v5의 본 정당화(다중 인스턴스 경계
  정합)와 v2(JVM 모니터)의 다중 JVM 붕괴는 **단일 JVM 측정으론 실증 불가**. 보강#1(Docker Compose N인스턴스 +
  Nginx LB + 공유 Redis로 v2 붕괴 + v3~v5 경계초월 정합 실측)에서만 승격 — 이번 스코프 밖, 다음 후보로 명시.
- **핀 검증 비대칭**(위 통제변수 섹션): pool/connTO 2종 결정적 실측 / Tomcat 1종 무해 미검증.
- **ⓐ/ⓑ 음성**: v3 503 분류는 배선·풀포화까지 검증됐으나 이 핀·부하서 미주행(타임아웃 미발생) — 조건부 미검증.
