...
- **목록 + 페이징: v3 @BatchSize.** fetch join은 1:N+페이징에서 메모리 페이징(HHH000104)이라 막힘(아래 §7) → IN 배치가 부작용 없이 거의 모든 조회에 꽂힌다. 비페이징 풀목록 p95는 v3가 v4/v5보다 느리지만, **페이징 시 N이 작아져** 쿼리·메모리가 page size에 고정(예측 가능).
